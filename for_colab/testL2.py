...
from synthetic_data.data_epidemics import epidemics_generator_mse, load_from_npy, save_to_npy
...
def _fill_test_feed_dict(mb_source, x, y, dropout, phase, is_training):
    (data, labels), is_end = mb_source.next_batch(FLAGS.num_test)
    feed_dict = {x: data, y: labels, dropout: 0.5 if is_training else 1, phase: is_training}
    still_data = not is_end
    return feed_dict, still_data
...
def run_training(L, train_mb_source, test_mb_source):
    ...
    x = tf.compat.v1.placeholder(tf.float32, [None, num_vertices, FLAGS.num_frames, 1], name="x")
