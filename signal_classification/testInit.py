...
from signal_classification.models2 import deep_fir_tv_fc_fn, fc_fn, \
    deep_cheb_fc_fn, deep_sep_fir_fc_fn, deep_fir_tv_zero
...
def run_training(L, train_mb_source, test_mb_source):
    ...
    elif FLAGS.model_type == "deep_fir_zero":
        print("Training deep FIR-TV zero model...")
        logits, phase = deep_fir_tv_zero( x=x,
                                          num_classes=FLAGS.num_classes,
                                          time_filter_orders=FLAGS.time_filter_orders,
                                          vertex_filter_orders=FLAGS.vertex_filter_orders,
                                          num_filters=FLAGS.num_filters,
...
def main(_):
    ...
    if FLAGS.action == "train":
        ...
        #0 graph
        #W = np.zeros((FLAGS.num_vertices,FLAGS.num_vertices))
        #G = graphs.Graph(W)
    ...
    if FLAGS.action == "train":
        ...
        #L1 = tf.sparse.to_dense(L[1])
        #print(L1.shape)
        #print('yo')
        #with tf.Session() as sess:  print(L1.eval()) 
